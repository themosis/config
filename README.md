...
You can tell the reader to ignore some file extensions while parsing the directory. To do so, use the `ignore_reader()` method:
...
$reader->ignore_reader( new ReaderKey( 'yaml' ) );
$reader->ignore_reader( new ReaderKey( 'ini' ) );
```
...
As mentioned earlier, the first time you call the Config `get()` method, the values returned by a reader
are cached in configuration instance. If for any reason, the configuration values, where you read them from, are
changed in-between, either get a new instance of the config class or if you're working with an instance, you can
call the `refresh()` method.

The `refresh()` method will returned a new config instance for you with a fresh set of configuration values you can
read from:
